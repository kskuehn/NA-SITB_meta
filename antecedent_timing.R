...
data.nssi.thgts$timing <- ifelse(data.nssi.thgts$Study == "Kiekens et al., 2020", 0, ifelse(
  data.nssi.thgts$Study == "Kuehn et al., in Prep", 0, ifelse(
    data.nssi.thgts$Study == "Lear et al., 2019", 0, ifelse(
      data.nssi.thgts$Study == "Bresin et al., 2013", 0, ifelse(
        data.nssi.thgts$Study == "Selby et al., 2018", 1, 0
      )
    )
  )
))

data.nssi.thgts$timing <- as.factor(as.character(data.nssi.thgts$timing))

prior <- c(set_prior("normal(0, 1)", class = "b", coef = "Intercept"),
           set_prior("normal(0, 0.5)", class = "b", coef = "NSSI_thgts1"), 
           set_prior("normal(0, 0.5)", class = "b", coef = "timing1"),
           set_prior("normal(0, 0.5)", class = "b", coef = "NSSI_thgts1:timing1"),
           set_prior("student_t(3,0,2.5)", class="sd"),
           set_prior("student_t(3,0,2.5", class="sigma"))

model_nssi_thgts_antecedent_timing <-brm(NA.standard.CWP.lag ~ 0 + Intercept + NSSI_thgts*timing + ( 0 + Intercept | Study) + ( 0 + Intercept | Study:PID) + ( 0 + NSSI_thgts | Study) + ( 0 + NSSI_thgts | Study:PID), prior=prior, data=data.nssi.thgts, control = list(adapt_delta = 0.99, max_treedepth=15),
                                             sample_prior = TRUE, iter = 6000, chains = 4, backend = "cmdstanr", threads = threading(9))

summary(model_nssi_thgts_antecedent_timing)


data.nssi.bhx$timing <- ifelse(data.nssi.bhx$Study == "Armey et al., 2011", 1, ifelse(
  data.nssi.bhx$Study == "Kiekens et al., 2020", 1, ifelse(
    data.nssi.bhx$Study == "Kuehn et al., in Prep", 0, ifelse(
      data.nssi.bhx$Study == "Lear et al., 2019", 0, ifelse(
        data.nssi.bhx$Study == "Muehlenkamp et al., 2009", 0, ifelse(
                  ...
                  data.nssi.bhx$Study == "Vansteelandt et al., 2017", 1, ifelse(
                    data.nssi.bhx$Study == "Wolford-Clevenger et al., 2019", 0, ifelse(
                      data.nssi.bhx$Study == "Czyz et al., 2017", 0, ifelse(
                        data.nssi.bhx$Study == "Hochard et al., 2015", 0, 1)
                    )
                  )
                )
              )
...
data.nssi.bhx$timing <- as.factor(as.character(data.nssi.bhx$timing))

prior <- c(set_prior("normal(0, 1)", class = "b", coef = "Intercept"),
           set_prior("normal(0, 0.5)", class = "b", coef = "NSSI_bhx1"), 
           set_prior("normal(0, 0.5)", class = "b", coef = "timing1"),
           set_prior("normal(0, 0.5)", class = "b", coef = "NSSI_bhx1:timing1"),
           set_prior("student_t(3,0,2.5)", class="sd"),
           set_prior("student_t(3,0,2.5", class="sigma"))

model_NSSI_bhx_antecedent_timing <-brm(NA.standard.CWP.lag ~ 0 + Intercept + NSSI_bhx*timing + ( 0 + Intercept | Study) + ( 0 + Intercept | Study:PID) + ( 0 + NSSI_bhx | Study) + ( 0 + NSSI_bhx | Study:PID), prior=prior, data=data.nssi.bhx, control = list(adapt_delta = 0.99, max_treedepth=15),
                                        sample_prior = TRUE, iter = 6000, chains = 4, backend = "cmdstanr", threads = threading(9))

summary(model_NSSI_bhx_antecedent_timing)

data.sui.thgts$timing <- ifelse(data.sui.thgts$Study == "Kiekens et al., 2020", 0, ifelse(
  data.sui.thgts$Study == "Kleiman et al., 2017", 0, ifelse(
    data.sui.thgts$Study == "Kleiman et al., 2018", 0, ifelse(
      data.sui.thgts$Study == "Kuehn et al., in Prep", 0, ifelse(
        data.sui.thgts$Study == "Peters et al., 2020", 0, ifelse(
            ...
            data.sui.thgts$Study == "Bresin et al., 2013", 0, ifelse(
              data.sui.thgts$Study == "Wolford-Clevenger et al., 2019", 0, ifelse(
                data.sui.thgts$Study == "Czyz et al., 2017", 0, ifelse(
                  data.sui.thgts$Study == "Forkmann et al., 2018", 0, ifelse(
                    data.sui.thgts$Study == "Husky et al., 2017", 1, ifelse(
                      data.sui.thgts$Study == "Kaurin et al., under review", 0, 0)
                  )
                )
              )
            )
...
data.sui.thgts$timing <- as.factor(as.character(data.sui.thgts$timing))

prior <- c(set_prior("normal(0, 1)", class = "b", coef = "Intercept"),
           set_prior("normal(0, 0.5)", class = "b", coef = "suicidal_thgts1"), 
           set_prior("normal(0, 0.5)", class = "b", coef = "timing1"),
           set_prior("normal(0, 0.5)", class = "b", coef = "suicidal_thgts1:timing1"),
           set_prior("student_t(3,0,2.5)", class="sd"),
           set_prior("student_t(3,0,2.5", class="sigma"))

model_suicidal_thgts_antecedent_timing <-brm(NA.standard.CWP.lag ~ 0 + Intercept + suicidal_thgts*timing + ( 0 + Intercept | Study) + ( 0 + Intercept | Study:PID) + ( 0 + suicidal_thgts | Study) + ( 0 + suicidal_thgts | Study:PID), prior=prior, data=data.sui.thgts, control = list(adapt_delta = 0.99, max_treedepth=15),
                                              sample_prior = TRUE, iter = 6000, chains = 4, backend = "cmdstanr", threads = threading(9))

summary(model_suicidal_thgts_antecedent_timing)

save.image("antecedent_timing.RData", compress = "xz")