...
prior <- c(set_prior("normal(0, 1)", class = "b", coef = "Intercept"),
           ...
           set_prior("normal(0, 0.5)", class = "b", coef = "NSSI_thgts0:compliance"),
           set_prior("student_t(3,0,2.5)", class="sd"),
           set_prior("student_t(3,0,2.5", class="sigma"))

model_nssi_thgts_consequence_compliance <-brm(NA.standard.CWP ~ 0 + Intercept + NSSI_thgts*compliance + ( 0 + Intercept + NSSI_thgts | Study/PID/pair), prior=prior, data=data.nssi.thgts, control = list(adapt_delta = 0.99, max_treedepth=15),
                                   sample_prior = TRUE, iter = 6000, chains = 4, backend = "cmdstanr", threads = threading(9))

summary(model_nssi_thgts_consequence_compliance)
...
prior <- c(set_prior("normal(0, 1)", class = "b", coef = "Intercept"),
           ...
           set_prior("normal(0, 0.5)", class = "b", coef = "compliance"),
           set_prior("normal(0, 0.5)", class = "b", coef = "NSSI_bhx0:compliance"),
           set_prior("student_t(3,0,2.5)", class="sd"),
           set_prior("student_t(3,0,2.5", class="sigma"))

model_NSSI_bhx_consequence_compliance <-brm(NA.standard.CWP ~ 0 + Intercept + NSSI_bhx*compliance + ( 0 + Intercept + NSSI_bhx | Study/PID/pair), prior=prior, data=data.nssi.bhx, control = list(adapt_delta = 0.99, max_treedepth=15),
                                 sample_prior = TRUE, iter = 6000, chains = 4, backend = "cmdstanr", threads = threading(9))

summary(model_NSSI_bhx_consequence_compliance)
...
prior <- c(set_prior("normal(0, 1)", class = "b", coef = "Intercept"),
           ...
           set_prior("normal(0, 0.5)", class = "b", coef = "compliance"),
           set_prior("normal(0, 0.5)", class = "b", coef = "suicidal_thgts0:compliance"),
           set_prior("student_t(3,0,2.5)", class="sd"),
           set_prior("student_t(3,0,2.5", class="sigma"))

model_suicidal_thgts_consequence_compliance <-brm(NA.standard.CWP ~ 0 + Intercept + suicidal_thgts*compliance + ( 0 + Intercept + suicidal_thgts | Study/PID/pair), prior=prior, data=data.sui.thgts, control = list(adapt_delta = 0.99, max_treedepth=15),
                                       sample_prior = TRUE, iter = 6000, chains = 4, backend = "cmdstanr", threads = threading(9))

summary(model_suicidal_thgts_consequence_compliance)

save.image("consequence_compliance.RData", compress = "xz")