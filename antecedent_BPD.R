...
data.nssi.bhx$BPD <- ifelse(data.nssi.bhx$Study == "Armey et al., 2011", 0, ifelse(
  data.nssi.bhx$Study == "Kiekens et al., 2020", 0, ifelse(
      ...
      data.nssi.bhx$Study == "Lear et al., 2019", 0, ifelse(
        data.nssi.bhx$Study == "Muehlenkamp et al., 2009", 0, ifelse(
          data.nssi.bhx$Study == "Santangelo et al., 2017", 1, ifelse(
            data.nssi.bhx$Study == "Selby et al., 2013", 0, ifelse(
              data.nssi.bhx$Study == "Bresin et al., 2013", 0, ifelse(            
                data.nssi.bhx$Study == "Selby et al., 2018", 0, ifelse(
...
data.nssi.bhx$BPD <- as.factor(as.character(data.nssi.bhx$BPD))

prior <- c(set_prior("normal(0, 1)", class = "b", coef = "Intercept"),
           set_prior("normal(0, 0.5)", class = "b", coef = "NSSI_bhx1"), 
           set_prior("normal(0, 0.5)", class = "b", coef = "BPD1"),
           set_prior("normal(0, 0.5)", class = "b", coef = "NSSI_bhx1:BPD1"),
           set_prior("student_t(3,0,2.5)", class="sd"),
           set_prior("student_t(3,0,2.5", class="sigma"))

model_NSSI_bhx_antecedent_BPD <-brm(NA.standard.CWP.lag ~ 0 + Intercept + NSSI_bhx*BPD + ( 0 + Intercept | Study) + ( 0 + Intercept | Study:PID) + ( 0 + NSSI_bhx | Study) + ( 0 + NSSI_bhx | Study:PID), prior=prior, data=data.nssi.bhx, control = list(adapt_delta = 0.99, max_treedepth=15),
                                        sample_prior = TRUE, iter = 6000, chains = 4, backend = "cmdstanr", threads = threading(9))

summary(model_NSSI_bhx_antecedent_BPD)

data.sui.thgts$BPD <- ifelse(data.sui.thgts$Study == "Kiekens et al., 2020", 0, ifelse(
              ...
              data.sui.thgts$Study == "Wolford-Clevenger et al., 2019", 0, ifelse(
                data.sui.thgts$Study == "Czyz et al., 2017", 0, ifelse(
                  data.sui.thgts$Study == "Forkmann et al., 2018", 0, ifelse(
                    data.sui.thgts$Study == "Husky et al., 2017", 0, ifelse(
...
data.sui.thgts$BPD <- as.factor(as.character(data.sui.thgts$BPD))

prior <- c(set_prior("normal(0, 1)", class = "b", coef = "Intercept"),
           set_prior("normal(0, 0.5)", class = "b", coef = "suicidal_thgts1"), 
           set_prior("normal(0, 0.5)", class = "b", coef = "BPD1"),
           set_prior("normal(0, 0.5)", class = "b", coef = "suicidal_thgts1:BPD1"),
           set_prior("student_t(3,0,2.5)", class="sd"),
           set_prior("student_t(3,0,2.5", class="sigma"))

model_suicidal_thgts_antecedent_BPD <-brm(NA.standard.CWP.lag ~ 0 + Intercept + suicidal_thgts*BPD + ( 0 + Intercept | Study) + ( 0 + Intercept | Study:PID) + ( 0 + suicidal_thgts | Study) + ( 0 + suicidal_thgts | Study:PID), prior=prior, data=data.sui.thgts, control = list(adapt_delta = 0.99, max_treedepth=15),
                                              sample_prior = TRUE, iter = 6000, chains = 4, backend = "cmdstanr", threads = threading(9))
