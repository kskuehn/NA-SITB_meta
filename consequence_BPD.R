...
prior <- c(set_prior("normal(0, 1)", class = "b", coef = "Intercept"),
           ...
           set_prior("normal(0, 0.5)", class = "b", coef = "NSSI_bhx0:BPD1"),
           set_prior("student_t(3,0,2.5)", class="sd"),
           set_prior("student_t(3,0,2.5", class="sigma"))

model_NSSI_bhx_consequence_BPD <-brm(NA.standard.CWP ~ 0 + Intercept + NSSI_bhx*BPD + ( 0 + Intercept + NSSI_bhx | Study/PID/pair), prior=prior, data=data.nssi.bhx, control = list(adapt_delta = 0.99, max_treedepth=15),
                                 sample_prior = TRUE, iter = 6000, chains = 4, backend = "cmdstanr", threads = threading(9))

summary(model_NSSI_bhx_consequence_BPD)
...
prior <- c(set_prior("normal(0, 1)", class = "b", coef = "Intercept"),
           ...
           set_prior("normal(0, 0.5)", class = "b", coef = "BPD1"),
           set_prior("normal(0, 0.5)", class = "b", coef = "suicidal_thgts0:BPD1"),
           set_prior("student_t(3,0,2.5)", class="sd"),
           set_prior("student_t(3,0,2.5", class="sigma"))

model_suicidal_thgts_consequence_BPD <-brm(NA.standard.CWP ~ 0 + Intercept + suicidal_thgts*BPD + ( 0 + Intercept + suicidal_thgts | Study/PID/pair), prior=prior, data=data.sui.thgts, control = list(adapt_delta = 0.99, max_treedepth=15),
                                       sample_prior = TRUE, iter = 6000, chains = 4, backend = "cmdstanr", threads = threading(9))

summary(model_suicidal_thgts_consequence_BPD)

save.image("consequence_BPD.RData", compress = "xz")