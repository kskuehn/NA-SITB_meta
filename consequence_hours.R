...
prior <- c(set_prior("normal(0, 1)", class = "b", coef = "Intercept"),
           ...
           set_prior("normal(0, 0.5)", class = "b", coef = "NSSI_thgts0:hours.between.prompts"),
           set_prior("student_t(3,0,2.5)", class="sd"),
           set_prior("student_t(3,0,2.5", class="sigma"))

model_nssi_thgts_consequence_hours <-brm(NA.standard.CWP ~ 0 + Intercept + NSSI_thgts*hours.between.prompts + ( 0 + Intercept + NSSI_thgts | Study/PID/pair), prior=prior, data=data.nssi.thgts, control = list(adapt_delta = 0.99, max_treedepth=15),
                                   sample_prior = TRUE, iter = 6000, chains = 4, backend = "cmdstanr", threads = threading(9))

summary(model_nssi_thgts_consequence_hours)
...
prior <- c(set_prior("normal(0, 1)", class = "b", coef = "Intercept"),
           ...
           set_prior("normal(0, 0.5)", class = "b", coef = "hours.between.prompts"),
           set_prior("normal(0, 0.5)", class = "b", coef = "NSSI_bhx0:hours.between.prompts"),
           set_prior("student_t(3,0,2.5)", class="sd"),
           set_prior("student_t(3,0,2.5", class="sigma"))

model_NSSI_bhx_consequence_hours <-brm(NA.standard.CWP ~ 0 + Intercept + NSSI_bhx*hours.between.prompts + ( 0 + Intercept + NSSI_bhx | Study/PID/pair), prior=prior, data=data.nssi.bhx, control = list(adapt_delta = 0.99, max_treedepth=15),
                                 sample_prior = TRUE, iter = 6000, chains = 4, backend = "cmdstanr", threads = threading(9))

summary(model_NSSI_bhx_consequence_hours)
...
prior <- c(set_prior("normal(0, 1)", class = "b", coef = "Intercept"),
           ...
           set_prior("normal(0, 0.5)", class = "b", coef = "hours.between.prompts"),
           set_prior("normal(0, 0.5)", class = "b", coef = "suicidal_thgts0:hours.between.prompts"),
           set_prior("student_t(3,0,2.5)", class="sd"),
           set_prior("student_t(3,0,2.5", class="sigma"))

model_suicidal_thgts_consequence_hours <-brm(NA.standard.CWP ~ 0 + Intercept + suicidal_thgts*hours.between.prompts + ( 0 + Intercept + suicidal_thgts | Study/PID/pair), prior=prior, data=data.sui.thgts, control = list(adapt_delta = 0.99, max_treedepth=15),
                                       sample_prior = TRUE, iter = 6000, chains = 4, backend = "cmdstanr", threads = threading(9))

summary(model_suicidal_thgts_consequence_hours)

save.image("consequence_hours.RData", compress = "xz")